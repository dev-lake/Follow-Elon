...
class TwitterAPIClient:
    ...
    def __init__(self, api_key: str = None, base_url: str = None):
        ...
        self.base_url = base_url or 'https://api.twitterapi.io/v1/'
        
        if not self.api_key:
            raise AuthenticationError("API密钥未提供，请设置TWITTER_API_KEY环境变量或传入api_key参数")
    ...
    def _make_request(self, method: str, endpoint: str, params: Dict = None, data: Dict = None) -> Dict:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                raise AuthenticationError("认证失败，请检查API密钥", response.status_code, response.json())
                ...
                raise ValidationError("请求参数错误", response.status_code, response.json())
            else:
    ...
    def get_user_tweets(self, username: str, count: int = 20, include_replies: bool = False) -> Dict:
        """
        获取用户推文
    ...
    def get_user_retweets(self, username: str, count: int = 20) -> Dict:
        ...
        timeline_data = self._make_request('GET', '/statuses/user_timeline', params=params)
        
        # 过滤出转发的推文
        if isinstance(timeline_data, list):
            retweets = [tweet for tweet in timeline_data if tweet.get('retweeted_status')]
            return retweets
        elif isinstance(timeline_data, dict) and 'data' in timeline_data:
            retweets = [tweet for tweet in timeline_data['data'] if tweet.get('retweeted_status')]
    ...
    def search_tweets(self, query: str, count: int = 20, result_type: str = 'recent') -> Dict:
        """
        搜索推文
        ...
        if result_type not in ['recent', 'popular', 'mixed']:
            raise ValidationError("result_type必须是 'recent', 'popular' 或 'mixed' 之一")
